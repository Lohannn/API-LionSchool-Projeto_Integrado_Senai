...
app.get('/v1/lion-school/alunos/cursos', cors(), async function (request, response, next) {
    ...
    if (curso == '' || curso == undefined || !isNaN(curso)) {
        statusCode = 400
        dadosEstado.message = 'Não foi possível processar pois os dados de entrada (curso) que foram enviados não correspondem ao exigido, confira o valor pois não poder ser Vazio, e não pode conter letras.'
    } else {
